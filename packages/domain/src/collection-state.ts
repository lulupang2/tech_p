...
export type PartitionState = 'pending' | 'running' | 'deferred' | 'completed' | 'partial' | 'failed' | 'cancelled';
...
export interface CollectionWindow { readonly from: Date; readonly to: Date }
...
export interface CollectionStatePort {
  registerTarget(input: Omit<CollectionTargetRevision, 'id' | 'targetId' | 'configHash' | 'createdAt'>): Promise<CollectionTargetRevision>;
  ...
  claimPage(partitionId: string, pageSequence: number, now: Date, leaseMs: number): Promise<ClaimedCollectionPage | null>;
  renewPage(claim: ClaimedCollectionPage, now: Date, leaseMs: number): Promise<boolean>;
  commitPage(input: PageCommit): Promise<void>;
  failPage(claim: ClaimedCollectionPage, reason: string, retryAt: Date | null, now: Date): Promise<void>;
  resumePartition(id: string, now: Date): Promise<CollectionPartition>;
  claimDeliveries(now: Date, limit: number, leaseMs: number, mode?: CollectionMode): Promise<readonly CollectionDelivery[]>;
...
export class CollectionStateError extends Error {
  constructor(readonly code: 'invalid_window' | 'invalid_cursor' | 'stale_lease' | 'policy_blocked' | 'invalid_page' | 'not_found' | 'invalid_state') {
    super(code);
...
export function validateCollectionWindow(window: CollectionWindow): void {
  if (!Number.isFinite(window.from.getTime()) || !Number.isFinite(window.to.getTime()) || window.from >= window.to) {
...
export function assertCollectableTarget(target: CollectionTargetRevision): void {
  if (!target.enabled || !target.policy.approved || !target.policy.fetch || !target.policy.store ||
      target.capability.canCollect !== true || !target.capability.reviewedAt ||
      !Number.isFinite(Date.parse(target.capability.reviewedAt))) {
    throw new CollectionStateError('policy_blocked');
  }
}
export function encodePageCursor(partition: CollectionPartition, cursorVersion: number, value: string): string {
  return Buffer.from(JSON.stringify({ key: partitionNaturalKey(partitionPlan(partition)), cursorVersion, value }), 'utf8').toString('base64url');
}
export function partitionPlan(partition: PartitionPlan): PartitionPlan {
  return { targetRevisionId: partition.targetRevisionId, mode: partition.mode, scopeKey: partition.scopeKey,
    window: partition.window, timeBasis: partition.timeBasis, workflowVersion: partition.workflowVersion };
}
export function decodePageCursor(partition: CollectionPartition, cursorVersion: number): string | null {
  ...
  try {
    ...
    if (Object.keys(cursor).length !== 3 || cursor['key'] !== partitionNaturalKey(partitionPlan(partition)) ||
        cursor['cursorVersion'] !== cursorVersion || typeof cursor['value'] !== 'string') throw new Error();
    return cursor['value'];
  } catch { throw new CollectionStateError('invalid_cursor'); }
}
export function validatePageResult(partition: CollectionPartition, result: CollectionPageResult): void {
  if (!['continue', 'complete', 'deferred', 'partial'].includes(result.disposition)) throw new CollectionStateError('invalid_page');
  if (result.nextCursor !== null && (typeof result.nextCursor !== 'string' || result.nextCursor.length > 16384)) throw new CollectionStateError('invalid_cursor');
  if (result.retryAt !== null && (!(result.retryAt instanceof Date) || !Number.isFinite(result.retryAt.getTime()))) throw new CollectionStateError('invalid_page');
  if (result.reason !== null && !/^[a-z][a-z0-9_]{0,127}$/u.test(result.reason)) throw new CollectionStateError('invalid_page');
  if (result.disposition === 'continue' && (!result.nextCursor || result.nextCursor === partition.cursor)) throw new CollectionStateError('invalid_page');
  if (result.disposition === 'deferred' && !result.retryAt) throw new CollectionStateError('invalid_page');
  if (result.disposition === 'partial' && !result.reason) throw new CollectionStateError('invalid_page');
  if (!Number.isSafeInteger(result.requests) || result.requests < 0 || !Number.isSafeInteger(result.bytes) || result.bytes < 0) throw new CollectionStateError('invalid_page');
}
