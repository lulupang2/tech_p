...
function assertUsage(units: number, tokens: number): void {
  if (!Number.isSafeInteger(units) || units < 0 || !Number.isSafeInteger(tokens) || tokens < 0)
  ...
  return {
    async configureScope(scope) {
      ...
      for (const limit of Object.values(scope.laneLimits))
        assertUsage(limit.maxDailyUnits, limit.maxDailyTokens);
      if (
        !Array.isArray(scope.approvedModelProfiles) ||
        scope.approvedModelProfiles.some((hash) => !/^[a-f0-9]{64}$/u.test(hash))
      )
        throw new ProviderBudgetError('invalid_usage');
    ...
    async reserve(scopeId, attemptId, lane, units, tokens, now) {
      ...
      if (!attemptId || attemptId.length > 256 || !Number.isFinite(now.getTime()))
        throw new ProviderBudgetError('invalid_usage');
      ...
      return db.transaction(async (tx) => {
        const [scope] = await tx
          .select()
          .from(providerBudgetScopes)
          .where(eq(providerBudgetScopes.id, scopeId))
          .for('update');
        if (!scope?.approved || !scope.laneLimits[lane])
          throw new ProviderBudgetError('approval_required');
        const [existing] = await tx
          .select()
          .from(providerBudgetReservations)
          .where(eq(providerBudgetReservations.attemptId, attemptId));
        if (existing) {
          if (
        ...
        if (
          !row ||
          !laneLimit ||
          BigInt(row.daily_units) + BigInt(units) > BigInt(scope.maxDailyUnits) ||
          ...
          BigInt(row.lane_tokens) + BigInt(tokens) > BigInt(laneLimit.maxDailyTokens)
        )
          throw new ProviderBudgetError('budget_exhausted');
        const [reservation] = await tx
          .insert(providerBudgetReservations)
          .values({
            scopeId,
            attemptId,
            lane,
    ...
    async settle(id, units, tokens, now) {
      ...
      await db.transaction(async (tx) => {
        const [candidate] = await tx
          .select()
          .from(providerBudgetReservations)
          .where(eq(providerBudgetReservations.id, id));
        ...
        if (row.state === 'settled') {
          if (row.actualUnits !== units || row.actualTokens !== tokens)
            throw new ProviderBudgetError('reservation_conflict');
          return;
        }
        await tx
          .update(providerBudgetReservations)
          .set({ state: 'settled', actualUnits: units, actualTokens: tokens, updatedAt: now })
          .where(eq(providerBudgetReservations.id, id));
        if (units > row.units || tokens > row.tokens) {
          await tx
            .update(providerBudgetScopes)
            .set({ blocked: true })
            .where(eq(providerBudgetScopes.id, row.scopeId));
        }
      });
    },
    async holdUnknown(id, now) {
      await db
        ...
        .where(
          and(
            eq(providerBudgetReservations.id, id),
    ...
    async releaseUnsent(id, now) {
      await db.transaction(async (tx) => {
        ...
        const [work] = await tx
          ...
          .where(eq(embeddingWorkItems.reservationId, id));
        if (work && ['calling', 'completed', 'outcome_unknown'].includes(work.state))
