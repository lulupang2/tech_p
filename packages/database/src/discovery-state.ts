  ...
  return {
    async recordCandidate(candidate, now) {
      if (
        ...
        candidate.canonicalIdentity.length > 512 ||
      ...
      try {
        ...
        if (
          ...
          url.username ||
          url.password ||
          candidate.evidenceUrl.length > 2048
        )
          throw new Error();
      } catch {
        throw new CollectionStateError('invalid_state');
      ...
      return db.transaction(async (tx) => {
        const [source] = await tx
          .select()
          .from(sources)
          .where(eq(sources.key, candidate.sourceKey))
          .for('share');
        if (!source) throw new CollectionStateError('not_found');
        if (!source.enabled || !source.policyReviewedAt)
        ...
        const [row] = await tx
          ...
          .where(
            and(
              eq(discoveryCandidates.sourceId, source.id),
              eq(discoveryCandidates.canonicalIdentity, candidate.canonicalIdentity),
            ),
          );
        ...
        return {
          ...
          state: row.state as DiscoveryCandidateRecord['state'],
    ...
    async listCandidates(limit, after) {
      if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100)
        throw new CollectionStateError('invalid_state');
      const rows = await db
        .select({ candidate: discoveryCandidates, sourceKey: sources.key })
        .from(discoveryCandidates)
        .innerJoin(sources, eq(sources.id, discoveryCandidates.sourceId))
        .where(after ? sql`${discoveryCandidates.id} > ${after}::uuid` : undefined)
        .orderBy(asc(discoveryCandidates.id))
        .limit(limit);
      return rows.map(({ candidate, sourceKey }) => ({
        id: candidate.id,
        sourceKey: sourceKey as SourceKey,
        canonicalIdentity: candidate.canonicalIdentity,
        selector: candidate.selector,
        evidenceUrl: candidate.evidenceUrl,
        state: candidate.state as DiscoveryCandidateRecord['state'],
    ...
    async reviewCandidate(id, decision, targetId, actor, now) {
      if (
        ...
        actor.length > 128 ||
        !Number.isFinite(now.getTime()) ||
        (decision === 'accepted' ? targetId === null : targetId !== null)
      )
        throw new CollectionStateError('invalid_state');
      await db.transaction(async (tx) => {
        ...
        if (targetId) {
          ...
          if (
            ...
            target.canonicalIdentity !== candidate.canonicalIdentity
          )
            throw new CollectionStateError('invalid_state');
        }
        ...
        await tx
          ...
          .set({ state: decision, targetId, reviewedBy: actor, reviewedAt: now })
          .where(eq(discoveryCandidates.id, id));
      });
    },
  };
