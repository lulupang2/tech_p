...
function partitionRecord(row: PartitionRow): CollectionPartition {
  return {
    ...
    timeBasis: row.timeBasis as CollectionTimeBasis,
    workflowVersion: row.workflowVersion,
    state: row.state as PartitionState,
...
async function readTarget(
  ...
  id: string,
  lock = false,
): Promise<CollectionTargetRevision | null> {
  const query = db
    .select({ target: collectionTargets, revision: collectionTargetRevisions, source: sources })
    .from(collectionTargetRevisions)
    .innerJoin(collectionTargets, eq(collectionTargets.id, collectionTargetRevisions.targetId))
    .innerJoin(sources, eq(sources.id, collectionTargets.sourceId))
    .where(eq(collectionTargetRevisions.id, id));
  ...
  return {
    ...
    sourceKey: row.source.key as SourceKey,
    canonicalIdentity: row.target.canonicalIdentity,
    configHash: row.revision.configHash,
    selector: row.revision.selector,
    capability: row.revision.capability,
    policy: row.revision.policy,
    topicIds: row.revision.topicIds,
    cadenceMs: row.revision.cadenceMs,
    overlapMs: row.revision.overlapMs,
    taxonomyVersion: row.revision.taxonomyVersion,
    createdAt: row.revision.createdAt,
    enabled:
      row.source.enabled && row.target.enabled && row.target.currentRevisionId === row.revision.id,
  };
}
async function fencePage(
  db: Executor,
  ...
  if (
    !row ||
    row.targetRevisionId !== claim.target.id ||
    row.targetRevisionId !== claim.partition.targetRevisionId ||
    row.state !== 'running' ||
    row.pageSequence !== claim.partition.pageSequence ||
    row.leaseEpoch !== claim.partition.leaseEpoch ||
    !row.leaseUntil ||
    row.leaseUntil <= now ||
    row.runId !== claim.runId
  )
    throw new CollectionStateError('stale_lease');
  return row;
}
async function collectionDelivery(
  db: Executor,
  partitionId: string,
  pageSequence: number,
  notBefore: Date,
  ...
  await db
    ...
    .values({
      naturalKey: collectionHash({ kind: 'collection', partitionId, pageSequence }),
      kind: 'collection',
      partitionId,
      pageSequence,
      notBefore,
  ...
  return {
    async registerTarget(input) {
      if (
        ...
        !input.canonicalIdentity.trim() ||
        !Number.isSafeInteger(input.capability.cursorVersion) ||
        input.capability.cursorVersion < 1 ||
        input.topicIds.length > 32
      )
      ...
      return db.transaction(async (tx) => {
        ...
        if (input.policy.approved && !source.policyReviewedAt)
          throw new CollectionStateError('policy_blocked');
        await tx
          .insert(collectionTargets)
          .values({ sourceId: input.sourceId, canonicalIdentity: input.canonicalIdentity })
          .onConflictDoNothing();
        const [target] = await tx
          ...
          .where(
            and(
              eq(collectionTargets.sourceId, input.sourceId),
              eq(collectionTargets.canonicalIdentity, input.canonicalIdentity),
            ),
          )
          .for('update');
        if (!target) throw new CollectionStateError('not_found');
        await tx
          .insert(collectionTargetRevisions)
          .values({
            targetId: target.id,
            configHash,
            selector: input.selector,
        ...
        const [revision] = await tx
          ...
          .where(
            and(
              eq(collectionTargetRevisions.targetId, target.id),
              eq(collectionTargetRevisions.configHash, configHash),
            ),
          );
        ...
        await tx
          ...
          .set({
            ...
            ...(target.currentRevisionId !== revision.id ? { enabled: false } : {}),
          })
          .where(eq(collectionTargets.id, target.id));
        const result = await readTarget(tx, revision.id);
        if (!result) throw new CollectionStateError('not_found');
        return result;
      });
    },
    getTargetRevision(id) {
      return readTarget(db, id);
    },
    async setTargetEnabled(targetId, enabled, now) {
      await db.transaction(async (tx) => {
        const [row] = await tx
          .select()
          .from(collectionTargets)
          .where(eq(collectionTargets.id, targetId));
    ...
    async listTargets(limit, after) {
      if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100)
        throw new CollectionStateError('invalid_state');
      const rows = await db
        .select({ id: collectionTargets.currentRevisionId })
        .from(collectionTargets)
        .where(after ? sql`${collectionTargets.id} > ${after}::uuid` : undefined)
        .orderBy(asc(collectionTargets.id))
        .limit(limit);
      const revisions = await Promise.all(
        rows.map((row) => (row.id ? readTarget(db, row.id) : null)),
      );
    ...
    async planPartition(plan, now) {
      ...
      return db.transaction(async (tx) => {
        ...
        if (target.capability.timeBasis !== plan.timeBasis)
          throw new CollectionStateError('invalid_window');
        await tx
          .insert(collectionPartitions)
          .values({
        ...
        const [row] = await tx
          .select()
          .from(collectionPartitions)
          .where(eq(collectionPartitions.naturalKey, naturalKey));
        if (!row) throw new CollectionStateError('not_found');
        if (!['completed', 'partial', 'cancelled', 'failed'].includes(row.state))
          await collectionDelivery(tx, row.id, row.pageSequence, row.nextDueAt);
    ...
    async getPartition(id) {
      const [row] = await db
        .select()
        .from(collectionPartitions)
        .where(eq(collectionPartitions.id, id));
    ...
    async listPartitions(limit, after) {
      ...
      const rows = await db
        .select()
        .from(collectionPartitions)
        .where(after ? sql`${collectionPartitions.id} > ${after}::uuid` : undefined)
        .orderBy(asc(collectionPartitions.id))
        .limit(limit);
    ...
    async claimPage(partitionId, pageSequence, now, leaseMs) {
      ...
      return db.transaction(async (tx) => {
        ...
        const [candidate] = await tx
          .select()
          .from(collectionPartitions)
          .where(eq(collectionPartitions.id, partitionId));
        ...
        if (
          !row ||
          row.pageSequence !== pageSequence ||
          !['pending', 'running', 'deferred'].includes(row.state) ||
          row.nextDueAt > now ||
        ...
        if (row.runId)
          await tx
            ...
            .set({ status: 'failed', endedAt: now, errorSummary: 'lease_expired' })
            .where(and(eq(collectionRuns.id, row.runId), eq(collectionRuns.status, 'running')));
        await tx.insert(collectionPageAttempts).values({
          runId: run.id,
          partitionId: row.id,
          pageSequence: row.pageSequence,
        ...
        const [claimed] = await tx
          ...
          .where(eq(collectionPartitions.id, row.id))
          .returning();
    ...
    async commitPage({ claim, result, now }) {
      ...
      if (
        ...
        claim.partition.mode === 'backfill' &&
        ['feed_only', 'snapshot_only'].includes(claim.target.capability.historyMode)
      )
      ...
      await db.transaction(async (tx) => {
        ...
        decodePageCursor(
          { ...partitionRecord(row), cursor: result.nextCursor },
          target.capability.cursorVersion,
        );
        for (const item of result.items) {
          ...
          for (const permission of [
            ...
            if (itemRights[permission] !== undefined) {
              if (typeof itemRights[permission] !== 'boolean')
                throw new CollectionStateError('invalid_page');
          ...
          if (
            itemRights['licenseId'] !== undefined &&
            itemRights['licenseId'] !== rights.licenseId
          ) {
            if (itemRights['licenseId'] !== null && typeof itemRights['licenseId'] !== 'string')
              throw new CollectionStateError('invalid_page');
          ...
          if (!rights.approved || !rights.fetch || !rights.store)
            throw new CollectionStateError('policy_blocked');
          await tx
            .insert(rawItems)
            .values({
          ...
          const [raw] = await tx
            ...
            .where(
              and(
                eq(rawItems.sourceId, target.sourceId),
                eq(rawItems.externalId, item.externalId),
                eq(rawItems.payloadHash, item.rawHash),
              ),
            );
          ...
          const [revision] = await tx
            ...
            .where(eq(documentRevisions.rawItemId, raw.id))
        ...
        await tx.insert(collectionCheckpoints).values({
          partitionId: row.id,
          pageSequence: row.pageSequence,
          leaseEpoch: row.leaseEpoch,
          cursorBefore: row.cursor,
          ...
          retainedItems: result.items.length,
        ...
        await tx
          .update(collectionPartitions)
          .set({
            pageSequence: row.pageSequence + 1,
        ...
        await tx
          ...
          .set({
            ...
            cursorAfter: result.nextCursor,
            counts: {
              retained: result.items.length,
              requests: result.requests,
              bytes: result.bytes,
            },
          })
          .where(eq(collectionRuns.id, claim.runId));
        await tx
          .update(deliveryOutbox)
          .set({ completedAt: now })
          .where(
            and(
              eq(deliveryOutbox.partitionId, row.id),
              eq(deliveryOutbox.kind, 'collection'),
              eq(deliveryOutbox.pageSequence, row.pageSequence),
            ),
          );
        if (result.disposition === 'continue' || result.disposition === 'deferred')
          await collectionDelivery(tx, row.id, row.pageSequence + 1, nextDueAt);
      });
    },
    async failPage(claim, reason, retryAt, now) {
      await db.transaction(async (tx) => {
        ...
        await tx
          .update(collectionRuns)
          .set({ status: 'failed', endedAt: now, errorSummary: reason.slice(0, 128) })
          .where(eq(collectionRuns.id, claim.runId));
        await tx
          .update(deliveryOutbox)
          .set({ notBefore: retryAt ?? now, sentAt: null, completedAt: retryAt ? null : now })
          .where(
            and(
              eq(deliveryOutbox.partitionId, row.id),
              eq(deliveryOutbox.kind, 'collection'),
    ...
    async resumePartition(id, now) {
      return db.transaction(async (tx) => {
        ...
        const [resumed] = await tx
          .update(collectionPartitions)
          .set({ state: 'pending', nextDueAt: now, leaseUntil: null, reason: null, updatedAt: now })
          .where(eq(collectionPartitions.id, id))
          .returning();
        await collectionDelivery(tx, id, row.pageSequence, now);
        await tx
          .update(deliveryOutbox)
          .set({ completedAt: null, sentAt: null, notBefore: now })
          .where(
            and(
              eq(deliveryOutbox.partitionId, id),
              eq(deliveryOutbox.kind, 'collection'),
              eq(deliveryOutbox.pageSequence, row.pageSequence),
            ),
          );
      ...
      return db.transaction(async (tx) => {
        const rows = await tx
          ...
          .where(
            and(
              ...
              lte(deliveryOutbox.notBefore, now),
            ...
            WHERE p.id = ${deliveryOutbox.partitionId} AND s.enabled AND t.enabled
              ...
              or(
                isNull(deliveryOutbox.sentAt),
                lte(deliveryOutbox.sentAt, new Date(now.getTime() - 30000)),
              ),
            ),
          )
          .orderBy(asc(deliveryOutbox.notBefore), asc(deliveryOutbox.id))
          .limit(limit)
        ...
        for (const row of rows) {
          const [updated] = await tx
            .update(deliveryOutbox)
            .set({ leaseEpoch: row.leaseEpoch + 1, leaseUntil: new Date(now.getTime() + leaseMs) })
            .where(eq(deliveryOutbox.id, row.id))
            .returning();
          if (updated)
            claimed.push({ ...updated, kind: updated.kind as CollectionDelivery['kind'] });
        }
        return claimed;
      });
    },
    ...
    async markSent(id, leaseEpoch, now) {
      const updated = await db
        .update(deliveryOutbox)
        .set({ sentAt: now, leaseUntil: null })
        .where(
          and(
            eq(deliveryOutbox.id, id),
            eq(deliveryOutbox.leaseEpoch, leaseEpoch),
            sql`${deliveryOutbox.leaseUntil} > ${now}`,
            isNull(deliveryOutbox.completedAt),
          ),
        )
      ...
      await db.transaction(async (tx) => {
        ...
        if (delivery.kind === 'collection') {
          const checkpoints = await tx
            .select({ id: collectionCheckpoints.id })
            .from(collectionCheckpoints)
            .where(
              and(
                eq(collectionCheckpoints.partitionId, delivery.partitionId),
                eq(collectionCheckpoints.pageSequence, delivery.pageSequence),
              ),
            )
            .limit(1);
          ...
          const result =
            await tx.execute(sql`SELECT r.id FROM document_revisions r WHERE r.id = ${delivery.revisionId}
            ...
            AND NOT EXISTS (SELECT 1 FROM chunks c WHERE c.document_revision_id = r.id
              AND NOT EXISTS (SELECT 1 FROM embedding_work_items w
                JOIN embeddings e ON e.id = w.embedding_id
                WHERE w.chunk_id = c.id AND e.chunk_id = c.id
                AND w.input_hash = c.content_hash AND e.input_hash = c.content_hash
                AND e.profile_hash = ${collectionHash(profile)} AND e.dimensions = ${profile.dimensions}
        ...
        await tx
          .update(deliveryOutbox)
          .set({ completedAt: now, leaseUntil: null })
          .where(eq(deliveryOutbox.id, id));
      });
    },
    async attachRevision(rawItemId, revisionId, now) {
      await db.transaction(async (tx) => {
        const [revision] = await tx
          ...
          .from(documentRevisions)
          .where(
            and(eq(documentRevisions.id, revisionId), eq(documentRevisions.rawItemId, rawItemId)),
          );
        if (!revision) throw new CollectionStateError('not_found');
        await tx
          .update(acquisitionMemberships)
          .set({ revisionId })
          .where(eq(acquisitionMemberships.rawItemId, rawItemId));
        ...
        for (const membership of memberships) {
          await tx
            .insert(deliveryOutbox)
            .values({
              naturalKey: collectionHash({ kind: 'embedding', revisionId }),
              kind: 'embedding',
              partitionId: membership.partitionId,
              pageSequence: 0,
              revisionId,
              notBefore: now,
            })
...
/** Replays durable stage intent; Redis is populated by the worker outbox dispatcher. */
export function createDatabaseReplayPorts(db: Database): {
  targets: ReplayTargetPort;
  publisher: ReplayPublisherPort;
} {
  const scopeFilter = (scope: ReplayScope, targetId: string) =>
    scope === 'run' ? eq(acquisitionMemberships.runId, targetId) : eq(rawItems.id, targetId);
  return {
    targets: {
      async exists(scope, targetId) {
        const rows = await db
          .select({ id: rawItems.id })
          .from(rawItems)
          .innerJoin(acquisitionMemberships, eq(acquisitionMemberships.rawItemId, rawItems.id))
          .where(scopeFilter(scope, targetId))
          .limit(1);
      ...
      async isEnabled(scope, targetId) {
        const rows = await db
          ...
          .where(and(scopeFilter(scope, targetId), eq(sources.enabled, true)))
          .limit(1);
        return rows.length === 1;
      },
    },
        ...
        return db.transaction(async (tx) => {
          const rows = await tx
            .select({
              ...
              partitionId: collectionPartitions.id,
              pageSequence: collectionPartitions.pageSequence,
              workflowVersion: collectionPartitions.workflowVersion,
              targetRevisionId: collectionPartitions.targetRevisionId,
            })
            .from(rawItems)
            .innerJoin(acquisitionMemberships, eq(acquisitionMemberships.rawItemId, rawItems.id))
            .innerJoin(
              collectionPartitions,
              eq(collectionPartitions.id, acquisitionMemberships.partitionId),
            )
            .where(scopeFilter(request.scope, request.targetId))
            .limit(501);
          if (rows.length === 0 || rows.length > 500) {
            throw new InvalidReplayRequestError('Replay requires 1–500 acquired items');
          }
          let duplicate = true;
          const deliveryIds = new Set<string>();
          for (const row of rows) {
            const target = await readTarget(tx, row.targetRevisionId, true);
            if (!target) throw new CollectionStateError('not_found');
            assertCollectableTarget(target);
            const naturalKey = collectionHash({
              kind: 'normalization',
            ...
            const [delivery] = await tx
              .select()
              .from(deliveryOutbox)
              .where(eq(deliveryOutbox.naturalKey, naturalKey))
              .for('update');
            if (!delivery) throw new CollectionStateError('not_found');
            deliveryIds.add(delivery.id);
            if (inserted.length > 0) duplicate = false;
            if (!delivery.completedAt) {
              await tx
                ...
                .where(eq(deliveryOutbox.id, delivery.id));
            }
          }
          return { duplicate, deliveryIds: [...deliveryIds] };
        });
      },
    },
  };
