...
export const documentRevisions = pgTable(
  ...
  (table) => [
    ...
    index('document_revisions_title_fts_idx').using(
      'gin',
...
export const chunks = pgTable(
  ...
  (table) => [
    index('chunks_content_fts_idx').using('gin', sql`to_tsvector('simple', ${table.content})`),
...
export const collectionTargets = pgTable(
  'collection_targets',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    sourceId: uuid('source_id')
...
export const collectionTargetRevisions = pgTable(
  'collection_target_revisions',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    targetId: uuid('target_id')
      .notNull()
      .references(() => collectionTargets.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    ...
    check(
      'collection_target_cadence_valid',
      sql`${table.cadenceMs} IS NULL OR ${table.cadenceMs} >= 60000`,
    ),
    check('collection_target_overlap_valid', sql`${table.overlapMs} BETWEEN 0 AND 86400000`),
  ],
);

export const collectionPartitions = pgTable(
  'collection_partitions',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    targetRevisionId: uuid('target_revision_id')
      .notNull()
      .references(() => collectionTargetRevisions.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    ...
    check(
      'collection_partition_mode_valid',
      sql`${table.mode} IN ('backfill','incremental','on_demand')`,
    ),
    check(
      'collection_partition_basis_valid',
      sql`${table.timeBasis} IN ('published_at','updated_at','observed_at')`,
    ),
    check(
      'collection_partition_state_valid',
      sql`${table.state} IN ('pending','running','deferred','completed','partial','failed','cancelled')`,
    ),
    check(
      'collection_partition_counters_valid',
      sql`${table.pageSequence} >= 0 AND ${table.leaseEpoch} >= 0 AND ${table.attempts} >= 0`,
    ),
    index('collection_partition_due_idx').on(table.state, table.nextDueAt),
  ],
);
export const collectionPageAttempts = pgTable(
  'collection_page_attempts',
  {
    runId: uuid('run_id')
      .primaryKey()
    ...
    partitionId: uuid('partition_id')
      .notNull()
      .references(() => collectionPartitions.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    ...
    check(
      'collection_page_attempt_counters_valid',
      sql`${table.pageSequence} >= 0 AND ${table.leaseEpoch} > 0`,
    ),
  ],
);
...
export const collectionCheckpoints = pgTable(
  ...
  (table) => [
    ...
    check(
      'collection_checkpoint_disposition_valid',
      sql`${table.disposition} IN ('continue','complete','deferred','partial')`,
    ),
    check(
      'collection_checkpoint_counts_valid',
      sql`${table.retainedItems} >= 0 AND ${table.requests} >= 0 AND ${table.bytes} >= 0`,
    ),
  ],
);

export const acquisitionMemberships = pgTable(
  'acquisition_memberships',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    partitionId: uuid('partition_id')
      .notNull()
...
export const deliveryOutbox = pgTable(
  ...
  (table) => [
    ...
    check(
      'delivery_subject_valid',
      sql`(${table.kind} = 'collection' AND ${table.rawItemId} IS NULL AND ${table.revisionId} IS NULL) OR (${table.kind} = 'normalization' AND ${table.rawItemId} IS NOT NULL) OR (${table.kind} = 'embedding' AND ${table.revisionId} IS NOT NULL)`,
    ),
...
export const observationCohortMembers = pgTable(
  'observation_cohort_members',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    cohortId: uuid('cohort_id')
      .notNull()
      .references(() => observationCohorts.id, { onDelete: 'restrict' }),
    targetRevisionId: uuid('target_revision_id')
      .notNull()
      .references(() => collectionTargetRevisions.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    unique('cohort_member_unique').on(
      table.cohortId,
...
export const providerBudgetScopes = pgTable(
  'provider_budget_scopes',
  {
    ...
    approvedModelProfiles: jsonb('approved_model_profiles')
  ...
  (table) => [
    check(
      'budget_scope_limits_valid',
      sql`${table.maxDailyUnits} >= 0 AND ${table.maxOutstandingUnits} >= 0 AND ${table.maxDailyTokens} >= 0`,
    ),
  ],
);
export const providerBudgetReservations = pgTable(
  'provider_budget_reservations',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    scopeId: text('scope_id')
      .notNull()
      .references(() => providerBudgetScopes.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    check(
      'budget_reservation_state_valid',
      sql`${table.state} IN ('reserved','settled','outcome_unknown','released')`,
    ),
    check(
      'budget_reservation_usage_valid',
      sql`${table.units} >= 0 AND ${table.tokens} >= 0 AND (${table.actualUnits} IS NULL OR ${table.actualUnits} >= 0) AND (${table.actualTokens} IS NULL OR ${table.actualTokens} >= 0)`,
    ),
    index('budget_reservation_scope_day_idx').on(table.scopeId, table.utcDay, table.state),
  ],
);

export const embeddingWorkItems = pgTable(
  'embedding_work_items',
  {
    ...
    leaseEpoch: integer('lease_epoch').notNull().default(0),
    leaseUntil: timestamp('lease_until', { withTimezone: true, mode: 'date' }),
    reservationId: uuid('reservation_id').references(() => providerBudgetReservations.id, {
      onDelete: 'restrict',
    }),
  ...
  (table) => [
    ...
    check(
      'embedding_work_reservation_required',
      sql`${table.state} NOT IN ('calling','completed','outcome_unknown') OR ${table.reservationId} IS NOT NULL`,
    ),
    check(
      'embedding_work_state_valid',
      sql`${table.state} IN ('pending','claimed','calling','completed','outcome_unknown','failed')`,
    ),
    check(
      'embedding_work_completed_valid',
      sql`${table.state} != 'completed' OR ${table.embeddingId} IS NOT NULL`,
    ),
...
export const discoveryCandidates = pgTable(
  ...
  (table) => [
    ...
    check(
      'discovery_candidate_state_valid',
