...
describe('DB-002 schema migration', () => {
  test('defines raw revision identity and provenance constraints', () => {
    const migration = readFileSync(resolve(DEFAULT_MIGRATIONS_FOLDER, '0001_complete_puck.sql'), 'utf8');
  ...
  test('migration creates parent tables before foreign-key constraints', () => {
    const migration = readFileSync(resolve(DEFAULT_MIGRATIONS_FOLDER, '0001_complete_puck.sql'), 'utf8');
    assert.ok(migration.indexOf('CREATE TABLE "sources"') < migration.indexOf('ALTER TABLE "raw_items" ADD CONSTRAINT'));
    assert.ok(migration.indexOf('CREATE TABLE "raw_items"') < migration.indexOf('ALTER TABLE "pipeline_events" ADD CONSTRAINT'));
  });
});
