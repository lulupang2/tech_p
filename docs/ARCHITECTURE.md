...
## 5. 권장 배포 토폴로지
MVP 로컬 환경은 Docker Compose에서 `web`, `api`, `worker`, `postgres`, `redis`를 분리한다. 공유·운영 환경의 PostgreSQL provider는 Neon Serverless Postgres다([ADR-0011](./adr/0011-neon-serverless-postgresql.md)). production은 [ADR-0013](./adr/0013-production-deployment.md)에 따라 별도 `compose.production.yaml`에서 GHCR의 commit SHA 이미지를 사용한다. 기존 systemd Caddy는 저장소가 배포하는 `signal.jisung.lol` snippet을 검증·reload하고, `/api/*`, `/health/*`는 `127.0.0.1:3000`, 나머지는 `127.0.0.1:5173`으로 reverse proxy한다. Compose는 Caddy와 80/443을 소유하지 않아 기존 Discord routing을 보존한다.
...
| Production deployment | Compose + GHCR + SSH, managed container platform, host-local build | Docker Compose + GHCR + SSH, GitHub production approval, host Caddy snippet validation/reload, loopback API/web, SHA image rollback | **Accepted** — [ADR-0013](./adr/0013-production-deployment.md) |
