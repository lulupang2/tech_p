...
- 첫 화면(`/`)은 질문 입력과 예시 질문을 중심으로 한다. 기존 source/topic/status 탐색은 `/explore`에서 제공한다(2026-09-11 사용자 승인).
- 질문 홈은 상단 질문/탐색 메뉴, 짧은 기술별 예시와 실제 API 수집 현황을 제공한다. 시스템 상태는 하단에 둔다. 개별 최신 릴리스 목록 API가 없는 동안 고정 릴리스 제목·날짜를 최신 데이터처럼 표시하지 않는다(2026-09-11).
