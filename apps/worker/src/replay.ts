...
export function parseReplayJobData(value: unknown): ReplayJobData {
  ...
  if (
    job['schemaVersion'] !== 1 ||
    typeof job['replayId'] !== 'string' ||
    typeof job['naturalKey'] !== 'string' ||
    typeof job['targetId'] !== 'string' ||
    typeof job['requestedAt'] !== 'string'
  )
    throw new Error('Invalid replay job');
  if (
    !['run', 'raw', 'stage'].includes(String(job['scope'])) ||
    !['normalization', 'deduplication'].includes(String(job['stage']))
  )
    throw new Error('Invalid replay job');
  return job as unknown as ReplayJobData;
}

export function createReplayHandler(options: {
  readonly targets: ReplayTargetPort;
  readonly publisher: ReplayPublisherPort;
  readonly audit?: ReplayAuditPort;
}): (job: ReplayJobData) => Promise<ReplayResult> {
